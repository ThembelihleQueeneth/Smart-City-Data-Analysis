...
function validateSensorReadings(items) {
  ...
  items.forEach((item, index) => {
    ...
    if (value === null || value === undefined || isNaN(value) || 
        (typeof value === 'string' && isNaN(parseFloat(value.replace('C', ''))))) {
      console.log(`Item ${index} (sensor ${item.sensor_id}): Value "${value}" is not a valid number`);
    }
    
    // Check timestamp format
    if (!iso8601Regex.test(item.timestamp)) {
      console.log(`Item ${index} (sensor ${item.sensor_id}): Timestamp "${item.timestamp}" is not in ISO 8601 format`);
    }
  });
}

validateSensorReadings(items);
